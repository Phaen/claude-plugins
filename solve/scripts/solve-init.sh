...
node "${CLAUDE_PLUGIN_ROOT}/scripts/solve-tree.js" init "$SOLVE_ID" "$PWD"

# Write pointer file so the MCP server can find the active tree
echo "$SOLVE_ID" > "${PWD}/.claude/solve_current" "$TRANSCRIPT_LINE"
...
if ! curl -sf http://localhost:7337/state >/dev/null 2>&1; then
  PORT=7337 CLAUDE_PLUGIN_DATA="${CLAUDE_PLUGIN_DATA}" node "${CLAUDE_PLUGIN_ROOT}/build/index.js" </dev/null >/dev/null 2>&1 &
fi
